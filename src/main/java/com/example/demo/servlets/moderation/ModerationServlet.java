...
public class ModerationServlet extends HttpServlet {
    ...
    private void users(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SQLException {
        ...
        if (request.getMethod().equals("POST")) {
            ...
            if (action.equals("ban")) {
                ...
                UserDao.getInstance().unban(userId);
            } else if (action.equals("role_change")) {
                ...
                if (currentUser.getId().equals(userId)) {
                    request.getSession().setAttribute("alert", new Alert("danger", "You cannot change your own role."));
                    response.sendRedirect(request.getContextPath() + "/moderation?f=users");
                    return;
                }
                ...
                if (user.get().isAdministrator() || !currentUser.getId().equals(1)) {
                    request.getSession().setAttribute("alert", new Alert("danger", "The super-admin is the only one who can change the role of other admins."));
                    return;
                }
